...
def get_model(args, ingr_vocab_size, instrs_vocab_size):
    ...
    # build image model
    encoder_image = EncoderCNN(args.embed_size, args.dropout_encoder, args.image_model)#.cuda()
    ...
    model = InverseCookingModel(encoder_ingrs, decoder, ingr_decoder, encoder_image,
                                ...
                                label_smoothing=args.label_smoothing_ingr)#.cuda()
...
class InverseCookingModel(nn.Module):
    def __init__(self, ingredient_encoder, recipe_decoder, ingr_decoder, image_encoder,
        ...
        self.ingredient_encoder = ingredient_encoder.to(device)
        self.recipe_decoder = recipe_decoder.to(device)
        self.image_encoder = image_encoder.to(device)
        self.ingredient_decoder = ingr_decoder.to(device)
        self.crit = crit
    ...
    def forward(self, img_inputs, captions, target_ingrs,
        ...
        losses = {}
        #print("Min index:", target_ingrs.min().item(), "Max index:", target_ingrs.max().item())
        ...
        if not self.recipe_only:
            ...
            mask_perminv = mask_from_eos(target_ingrs, eos_value=0, mult_before=False)#.to(device) 
        ...
        # encode ingredients
        target_ingrs = target_ingrs.to(device)
        ...
        target_ingr_mask = target_ingr_mask.float().unsqueeze(1).to(device)
        img_features = img_features.to(device)
        captions  = captions.to(device)

        # print("target_ingr_feats shape:", target_ingr_feats.shape)
        # print("target_ingr_mask shape:", target_ingr_mask.shape)
